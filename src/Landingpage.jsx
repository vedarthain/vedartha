...
function MultiLangTitle() {
  ...
  return (
    <div style={{ textAlign: "center", marginBottom: 8 }}>
      ...
      {/* STATIC English — elegant serif with animated decorative strokes */}
      <svg viewBox="0 0 860 140" width="min(860px,92vw)" height="auto" style={{ display:"block", margin:"0 auto 2px", overflow:"visible" }}>
        <defs>
          <linearGradient id="gld" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="35%"  stopColor="#b9f6ca"/>
            <stop offset="65%"  stopColor="#00c853"/>
            <stop offset="100%" stopColor="#69f0ae"/>
          </linearGradient>
          <linearGradient id="strokeGrad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stopColor="#00c853" stopOpacity="0"/>
            <stop offset="20%"  stopColor="#00c853" stopOpacity="0.7"/>
            <stop offset="50%"  stopColor="#69f0ae" stopOpacity="0.9"/>
            <stop offset="80%"  stopColor="#00c853" stopOpacity="0.7"/>
          ...
          <style>{`
            ...
            .vt-main { font-family: 'Dancing Script', cursive; font-size: 112px; font-weight: 700;
        ...
        {/* Main elegant text */}
        <text x="50%" y="108" textAnchor="middle" className="vt-main">
          VEDARTHA
        </text>

        {/* Long animated underline with tapered ends */}
        ...
        {/* Thin second line slightly offset — double rule effect */}
        ...
        {/* Left serif extender — long horizontal stroke from V */}
        <line x1="10" y1="108" x2="90" y2="108"
          stroke="#00c853" strokeWidth="0.6" opacity="0"
          strokeDasharray="90" strokeDashoffset="90"
          style={{ animation: "drawLine 0.8s ease 1.6s forwards", opacity: 0.3 }}
        />

        {/* Right serif extender — long horizontal stroke from A */}
        <line x1="770" y1="108" x2="850" y2="108"
          stroke="#00c853" strokeWidth="0.6"
          strokeDasharray="90" strokeDashoffset="90"
          style={{ animation: "drawLine 0.8s ease 1.6s forwards", opacity: 0.3 }}
        />

        {/* Accent dots at underline ends */}
        <circle cx="30"  cy="124" r="0" fill="#69f0ae"
          style={{ animation: "dotPop 0.4s ease 2.7s forwards" }}/>
        <circle cx="830" cy="124" r="0" fill="#69f0ae"
          style={{ animation: "dotPop 0.4s ease 2.7s forwards" }}/>

        {/* Center diamond accent */}
        <polygon points="430,118 434,124 430,130 426,124"
          fill="#00c853" opacity="0"
          style={{ animation: "vtFadeUp 0.5s ease 2.9s both", opacity: 0.5 }}
        />
      </svg>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, margin:"6px auto 14px", width:"min(320px,70vw)" }}>
        <div style={{ flex:1, height:1, background:"linear-gradient(90deg,transparent,rgba(0,200,83,0.45))" }}/>
        <div style={{ display:"flex", gap:5, alignItems:"center" }}>
          <div style={{ width:3, height:3, borderRadius:"50%", background:"rgba(0,200,83,0.5)" }}/>
          <div style={{ width:5, height:5, transform:"rotate(45deg)", background:"rgba(0,200,83,0.65)", borderRadius:1 }}/>
          <div style={{ width:3, height:3, borderRadius:"50%", background:"rgba(0,200,83,0.5)" }}/>
        </div>
        <div style={{ flex:1, height:1, background:"linear-gradient(90deg,rgba(0,200,83,0.45),transparent)" }}/>
      </div>
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      lines.forEach((l) => {
        ...
        grad.addColorStop(0.5, `rgba(0,200,83,${l.opacity})`);
...
function Ticker() {
  ...
  return (
    <div style={{ background: "rgba(2,13,6,0.85)", borderTop: "1px solid rgba(0,200,83,0.2)", borderBottom: "1px solid rgba(0,200,83,0.2)", overflow: "hidden", whiteSpace: "nowrap", padding: "8px 0" }}>
      <div style={{ display: "inline-flex", gap: 0, animation: "ticker 40s linear infinite" }}>
        {items.map((item, i) => (
          <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "0 28px", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <span style={{ fontSize: 10, color: item.up ? "#00c853" : "#00c853", fontFamily: "'Courier Prime',monospace", fontWeight: 700 }}>{item.up ? "▲" : "▼"} {item.change}</span>
          </span>
        ))}
      </div>
    </div>
    ...
    <div style={{ minHeight: "100vh", background: "#020d06", color: "#e8f5e9", fontFamily: "'Nunito',sans-serif", overflowX: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:#020d06}::-webkit-scrollbar-thumb{background:#00c85355;border-radius:2px}
        @keyframes ticker{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
        @keyframes fadeUp{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(0,200,83,0.4),0 0 60px rgba(0,200,83,0.1)}50%{box-shadow:0 0 40px rgba(0,200,83,0.7),0 0 100px rgba(0,200,83,0.2)}}
        ...
        .stat-num{font-family:'Courier Prime',monospace;font-size:28px;font-weight:700;color:#00e676;line-height:1;}
      `}</style>

      <StarField />

      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "16px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(180deg,rgba(2,13,6,0.95),rgba(2,13,6,0))", backdropFilter: "blur(8px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 36, height: 36, background: "linear-gradient(135deg,#00c853,#00e676)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#020d06", fontWeight: 900, fontFamily: "'Outfit',serif", boxShadow: "0 0 16px rgba(0,200,83,0.4)" }}>वे</div>
          <div>
            <div style={{ fontFamily: "'Outfit',serif", fontSize: 14, fontWeight: 700, color: "#e8f5e9", letterSpacing: ".08em" }}>VEDARTHA<span style={{ color: "#00c853" }}>.IN</span></div>
            <div style={{ fontSize: 7, color: "rgba(0,200,83,0.5)", letterSpacing: ".2em", fontFamily: "'Nunito',sans-serif" }}>KNOWLEDGE OF WEALTH</div>
          </div>
        </div>
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "140px 24px 80px", position: "relative", zIndex: 1 }}>
        ...
        {/* Decorative ring */}
        <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", border: "1px solid rgba(0,200,83,0.06)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", animation: "rotateSlow 60s linear infinite", pointerEvents: "none" }} />
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", border: "1px solid rgba(0,200,83,0.08)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", animation: "rotateSlow 40s linear infinite reverse", pointerEvents: "none" }} />

        {/* Badge */}
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 18px", background: "rgba(0,200,83,0.1)", border: "1px solid rgba(0,200,83,0.3)", borderRadius: 30, marginBottom: 32, animation: `fadeUp 0.6s ease 0.2s both` }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#00c853", animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 9, color: "#00c853", letterSpacing: ".2em", fontFamily: "'Nunito',sans-serif", fontWeight: 700 }}>NSE · BSE · LIVE MARKET DATA</span>
        </div>

        {/* Multilanguage cycling headline */}
        <div style={{ animation: "fadeUp 0.7s ease 0.3s both", marginBottom: 24 }}>
          <MultiLangTitle />
        </div>
        ...
        <p style={{ fontSize: "clamp(13px,2vw,17px)", color: "rgba(232,245,233,0.55)", maxWidth: 520, lineHeight: 1.7, marginBottom: 48, fontFamily: "'Nunito',sans-serif", fontWeight: 500, animation: `fadeUp 0.7s ease 0.5s both` }}>
          The ancient pursuit of <em style={{ color: "#00c853", fontStyle: "normal" }}>Artha</em> — wealth and meaning — meets the precision of modern market intelligence. Your complete window into Indian equities.
        </p>
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap", justifyContent: "center", animation: `fadeUp 0.7s ease 0.6s both` }}>
          ...
          <button style={{ background: "transparent", border: "1px solid rgba(232,245,233,0.15)", borderRadius: 50, padding: "16px 36px", color: "rgba(232,245,233,0.6)", fontFamily: "'Nunito',sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: ".1em", cursor: "pointer", transition: "all 0.3s" }}
            onMouseEnter={e => { e.target.style.borderColor = "rgba(0,200,83,0.5)"; e.target.style.color = "#00e676"; }}
        ...
        <div style={{ position: "absolute", bottom: 32, left: "50%", transform: "translateX(-50%)", animation: "float 2.5s ease-in-out infinite", opacity: 0.3 }}>
          <div style={{ width: 1, height: 48, background: "linear-gradient(180deg,transparent,#00c853)", margin: "0 auto 4px" }} />
          <div style={{ fontSize: 8, color: "#00c853", letterSpacing: ".2em", fontFamily: "'Nunito',sans-serif" }}>SCROLL</div>
        </div>
      </section>

      {/* STATS BAR */}
      <section style={{ position: "relative", zIndex: 1, borderTop: "1px solid rgba(0,200,83,0.1)", borderBottom: "1px solid rgba(0,200,83,0.1)", background: "rgba(2,13,6,0.7)", backdropFilter: "blur(10px)", padding: "32px 40px" }}>
      ...
      <section style={{ position: "relative", zIndex: 1, padding: "80px 40px", maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          <div style={{ fontSize: 9, color: "#00c853", letterSpacing: ".25em", fontFamily: "'Nunito',sans-serif", fontWeight: 700, marginBottom: 14 }}>WHAT'S INSIDE</div>
          <h2 style={{ fontFamily: "'Outfit',serif", fontSize: "clamp(24px,4vw,42px)", fontWeight: 700, color: "#e8f5e9", lineHeight: 1.2 }}>
            Everything you need to<br />
            <span style={{ color: "#00c853" }}>read the market</span>
          </h2>
        </div>
      ...
      <section style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "80px 24px 100px" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <div style={{ width: 60, height: 60, background: "linear-gradient(135deg,#00c853,#00e676)", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, color: "#020d06", fontWeight: 900, fontFamily: "'Outfit',serif", margin: "0 auto 24px", boxShadow: "0 0 30px rgba(0,200,83,0.35)", animation: "glow 3s infinite" }}>वे</div>
          <h2 style={{ fontFamily: "'Outfit',serif", fontSize: "clamp(22px,4vw,38px)", fontWeight: 700, color: "#e8f5e9", marginBottom: 16, lineHeight: 1.3 }}>
            Begin your journey into<br /><span style={{ color: "#00c853" }}>Indian market wisdom</span>
          </h2>
      ...
      {/* FOOTER */}
      <footer style={{ position: "relative", zIndex: 1, borderTop: "1px solid rgba(0,200,83,0.1)", padding: "24px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, background: "rgba(2,13,6,0.7)" }}>
        <div style={{ fontFamily: "'Outfit',serif", fontSize: 12, fontWeight: 700, color: "rgba(232,245,233,0.25)", letterSpacing: ".08em" }}>VEDARTHA<span style={{ color: "rgba(0,200,83,0.3)" }}>.IN</span></div>
        ...
        <div style={{ fontSize: 9, color: "rgba(0,200,83,0.3)", fontFamily: "'Courier Prime',monospace", letterSpacing: ".1em" }}>वेदार्थ · KNOWLEDGE OF WEALTH</div>
      </footer>
    </div>
  );
}
